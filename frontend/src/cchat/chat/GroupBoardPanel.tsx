...
export function GroupBoardPanel({ chat, onBack }: { chat: any; onBack: () => void }) {
  ...
  return (
    <div className="flex w-[340px] shrink-0 flex-col bg-[#1e1f22] border-l border-[#2b2d31] h-full overflow-hidden text-[#e1e1e1]">
      <div className="flex h-[65px] items-center border-b border-[#2b2d31] px-4 py-[14px] shrink-0 font-medium text-[16px] text-white">
        <button
          onClick={onBack}
          className="mr-3 text-[#a1a1a1] hover:text-[#e1e1e1] transition-colors p-1 -ml-1 rounded-full hover:bg-[#2b2d31]"
        >
      ...
      <div className="flex border-b border-[#2b2d31] px-4 pt-2">
      ...
      <div className="flex-1 overflow-y-auto min-h-0 custom-scrollbar bg-[#111214] p-4">
        {filteredItems.length === 0 ? (
          ...
          <div className="flex flex-col gap-4">
            {filteredItems.map((item) => (
              <div key={item._id} className="bg-[#1e1f22] rounded-lg p-3 border border-[#2b2d31] shadow-sm">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  />
                  <span className="font-semibold text-sm text-white">{item.senderId?.fullname || "Người dùng"}</span>
                ...
                {item.messageType === "note" ? (
                   ...
                   <div className="text-sm text-[#e1e1e1] break-words">
      ...
      {(activeTab === "notes" || activeTab === "all" || activeTab === "polls") && (canCreateNote || canCreatePoll) && (
        <div className="p-4 border-t border-[#2b2d31] bg-[#1e1f22] flex gap-2 shrink-0">
          {(activeTab === "notes" || activeTab === "all") && canCreateNote && (
            <button
              onClick={() => setIsCreateNoteOpen(true)}
              className="flex-1 flex items-center justify-center gap-2 bg-[#2b2d31] hover:bg-[#3f4147] text-white py-2 rounded-md font-medium text-sm transition-colors"
            >
    ...
    <button
      ...
      className={`pb-2 px-1 text-[14px] font-medium transition-colors border-b-2 whitespace-nowrap flex-1 text-center ${
        active
          ...
          : "border-transparent text-[#a1a1a1] hover:text-[#e1e1e1]"
      }`}
    >
      {children}
    </button>
