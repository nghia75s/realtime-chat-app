        ...
        <div className="max-w-5xl mx-auto space-y-6">
          ...
          <div className="bg-[#1e1f22] border border-[#2b2d31] rounded-xl overflow-hidden shadow-sm">
            <div className="p-4 border-b border-[#2b2d31] flex items-center gap-2 bg-[#2b2d31]/20">
              ...
              <h3 className="text-lg font-bold text-white">Ban Lãnh Đạo</h3>
            </div>
            <div className="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {leaders.length === 0 ? (
                <div className="text-sm text-[#a1a1a1] col-span-full">Chưa có lãnh đạo nào.</div>
              ) : (
                leaders.map(leader => (
                  <div key={`leader-${leader._id}`} className="flex items-center gap-4 p-3 bg-[#131416] border border-[#2b2d31] rounded-lg">
                    <img src={leader.profilePicture || "/avatar.png"} alt={leader.fullname} className="w-12 h-12 rounded-full object-cover" />
                    <div>
                      <h4 className="font-medium text-white text-[15px]">{leader.fullname}</h4>
          ...
          {DEPARTMENTS.map(deptName => {
            ...
            return (
              <div key={deptName} className="bg-[#1e1f22] border border-[#2b2d31] rounded-xl overflow-hidden shadow-sm transition-all duration-200">
                {/* Accordion Header */}
                <div
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-[#a1a1a1] mt-0.5">{deptMembers.length} nhân sự</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <div className="hidden md:flex items-center gap-3 text-right">
                      <div>
                        ...
                        <p className="text-sm text-[#e1e1e1] font-medium">{manager?.fullname || "Trống"}</p>
                      </div>
                      <img src={manager?.profilePicture || "/avatar.png"} alt={manager?.fullname || "Trống"} className="w-8 h-8 rounded-full object-cover border border-[#2b2d31]" />
                    </div>
                  ...
                  <div className="bg-[#131416]/50 animate-in slide-in-from-top-2 duration-200">
                    ...
                    <div className="divide-y divide-[#2b2d31]">
                      {deptMembers.map((member) => (
                        <div key={member._id} className="flex items-center justify-between p-4 hover:bg-[#2b2d31]/50 transition-colors">
                          <div className="flex items-center gap-4">
