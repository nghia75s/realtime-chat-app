...
export function TaskHeader() {
  ...
  return (
    <div className="flex items-center justify-between px-6 py-4 border-b shrink-0 h-[64px]" style={{ background: 'var(--chat-bg-header)', borderColor: 'var(--chat-border)' }}>
      ...
      <div className="flex items-center gap-4 relative">
        <div ref={dropdownRef} className="relative">
          ...
          >
            ...
            {unreadCount > 0 && (
              <span className="absolute top-1 right-1 w-4 h-4 text-[10px] flex items-center justify-center rounded-full bg-red-500 text-white font-bold border border-chat-border" style={{ borderColor: 'var(--chat-bg-header)' }}>
          ...
          {showDropdown && (
            <div className="absolute right-0 mt-2 w-[350px] border rounded-xl shadow-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200" style={{ background: 'var(--chat-dropdown-bg)', borderColor: 'var(--chat-border)' }}>
              <div className="flex items-center justify-between p-3 border-b" style={{ borderColor: 'var(--chat-border)' }}>
                ...
                {notifications.length > 0 ? (
                  notifications.map((n) => (
                    <div
                      ...
                      className={`flex gap-3 p-3 border-b hover:bg-chat-hover cursor-pointer transition-colors ${!n.isRead ? 'bg-[#0052cc]/5' : ''}`}
                      style={{ borderColor: 'var(--chat-border)' }}
                    >
                      <img src={n.sender.profilePicture || "/avatar.png"} className="w-8 h-8 rounded-full object-cover shrink-0" style={{ background: 'var(--chat-bg-sidebar)' }} />
                      <div className="flex flex-col gap-1 flex-1">
                        <p className={`text-[13px] leading-tight ${!n.isRead ? 'text-chat-text font-medium' : 'text-chat-muted'}`}>{n.message}</p>
